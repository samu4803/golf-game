...
void ballAI::move(float initballX,float initballY,float initmouseX,float initmouseY,SDL_Renderer *render,SDL_Texture *texture)
{
  ...
  if(initballX<initmouseX&&initballY>initmouseY) //uprt->downlt
  direction=5;
  else
  if(initballX<initmouseX&&initballY<initmouseY) //downrt->uplt
  direction=6;
  else
  if(initballX>initmouseX&&initballY<initmouseY) // downlt->uprt
  direction=7;
  else
  if(initballX==initmouseX&&initballY>initmouseY)// up->down
  direction=0;
  if(initballX==initmouseX&&initballY<initmouseY)// down->up
  direction=1;
  else
  if(initballX>initmouseX&&initballY==initmouseY)// left->right
  direction=2;
  else
  if(initballX>initmouseX&&initballY==initmouseY)// right->left
  direction=3;
  std::cout<<"\n direction ="<<direction;
  std::cout<<"distance=="<<d;
